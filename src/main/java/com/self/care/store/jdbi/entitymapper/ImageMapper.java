...
public class ImageMapper implements ResultSetMapper<ImageBean> {
	...
	public ImageBean map(int index, ResultSet rs, StatementContext ctx)
		...
		try {
			ib.setCountry(EnumCountryCache.getInstance().getValue(enumCountryId).clone());
		...
		ib.setUserId(userId);
		try {
			ib.setUser(UserCache.getInstance().getValue(userId).clone());
		...
		try {
			ib.setRating(EnumRatingCache.getInstance().getValue(enumRatingId.toString()).clone());
		} catch (ExecutionException e) {
